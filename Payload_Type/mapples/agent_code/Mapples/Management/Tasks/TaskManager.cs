...
namespace Mapples.Management.Tasks
{
    public class TaskManager : ITaskManager
    {
        ...
        {
            // Annoying note - if there's an assembly in the Tasks DLL that isn't in the Mapples
        ...
        public bool ProcessMessageResponse(MessageResponse resp)
        {
            ...
            if (resp.Responses != null && resp.Responses.Length > 0)
            {
                ...
                {
                    if (_agent.GetFileManager().GetPendingTransfers().Contains(t.MapplesTrackerUUID))
                    {
