...
using MapplesInterop.Classes.Events;
...
using MapplesInterop.Classes.Cryptography;
using EncryptedFileStore;

namespace Mapples.Management.Files
{
    public sealed class FileManager : IFileManager
    {
        ...
        {
            if (_uploadMessageStore.ContainsKey(resp.MapplesTrackerUUID))
            {
                // This is an upload message response, send it along.
                if (resp.ChunkNumber > 0 && _uploadMessageStore.ContainsKey(resp.MapplesTrackerUUID))
                {
                    _uploadMessageStore[resp.MapplesTrackerUUID].MessageStore.AddMessage(resp);
                }
            } else
            {
                if (_downloadMessageStore.ContainsKey(resp.MapplesTrackerUUID))
                {
                    _downloadMessageStore[resp.MapplesTrackerUUID].AddMessage(resp);
                }
            }
        }
        ...
        private void FileManager_MessageComplete(object sender, ChunkMessageEventArgs<TaskStatus> e)
        {
            ...
            if (_uploadMessageStore.TryGetValue(e.Chunks[0].MapplesTrackerUUID, out UploadMessageTracker tracker))
            {
                tracker.Data = data.ToArray();
                _uploadMessageStore[e.Chunks[0].MapplesTrackerUUID] = tracker;
        ...
        private void MessageStore_ChunkAdd(object sender, ChunkMessageEventArgs<TaskStatus> e)
        {
            ...
            _agent.GetTaskManager().AddTaskResponseToQueue(new TaskResponse()
            {
                ...
                MapplesTrackerUUID = msg.MapplesTrackerUUID
            });
        }
        ...
        private void DownloadChunkSent(object sender, ChunkMessageEventArgs<TaskStatus> e)
        {
            ...
            _agent.GetTaskManager().AddTaskResponseToQueue(new TaskResponse()
            {
                ...
                MapplesTrackerUUID = e.Chunks[0].MapplesTrackerUUID
            });
        }
