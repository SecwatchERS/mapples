...
INTEROP_ASSEMBLY_PATH = "/srv/MapplesInterop.dll"
INTEROP_FILE_ID = ""
...
class InlineAssemblyCommand(CommandBase):
    ...
    async def build_interop(self):
        ...
        outputPath = "{}/MapplesInterop/bin/Release/MapplesInterop.dll".format(agent_build_path.name)
        copy_tree(self.agent_code_path, agent_build_path.name)
        shell_cmd = "rm -rf packages/*; nuget restore -NoCache -Force; msbuild -p:Configuration=Release {}/MapplesInterop/MapplesInterop.csproj".format(agent_build_path.name)
        proc = await asyncio.create_subprocess_shell(shell_cmd, stdout=asyncio.subprocess.PIPE,
                                                         stderr=asyncio.subprocess.PIPE, cwd=agent_build_path.name)
        stdout, stderr = await proc.communicate()
        if not path.exists(outputPath):
            raise Exception("Failed to build MapplesInterop.dll:\n{}".format(stderr.decode()))
