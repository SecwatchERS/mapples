    ...
    if (files.length < FILE_THRESHOLD)
    {
      for (var j = 0; j < files.length; j++)
      {
        var additionalInfo = btoa(files[j]['name']) + "|" + btoa(files[j]['full_name']) + "|" + btoa(data['host']);
          copyicon = '<i class="fas fa fa-clipboard" data-toggle="tooltip" title="Copy filename to clipboard" additional-info=' + btoa(files[j]['full_name']) + ' style="cursor: pointer;" onclick=support_scripts[\"mapples_copy_additional_info_to_clipboard\"](this)></i>';   
          if (files[j]["is_file"]){
            ...
            rows.push({ "": copyicon,
                        ...
                        "size": support_scripts['mapples_file_size_to_human_readable_string'](files[j]['size']),
                        "last accessed": files[j]["access_time"],
                        "last modified": files[j]["modify_time"],
                        "creation date": files[j]["creation_date"],
                        "acls": '<i class="fas fa-users" data-toggle="tooltip" title="Access Control List for ' + files[j]['name'] + '" style="cursor: pointer;" modal-name="' + uniqueName + '" additional-info="' + additionalInfo + '" onclick=support_scripts[\"mapples_show_permission_additional_info_modal\"](this)></i>',
                        "owner": files[j]['owner'],
                        "row-style": row_style,
            ...
            "size": support_scripts['mapples_file_size_to_human_readable_string'](0),
            "last modified": files[j]["modify_time"],
            "last accessed": files[j]["access_time"],
            "creation date": files[j]["creation_date"],
            "acls": '<i class="fas fa-users" style="cursor: pointer;" data-toggle="tooltip" title="Access Control List for ' + files[j]['name'] + '" modal-name="' + uniqueName + '" additional-info="' + additionalInfo + '" onclick=support_scripts[\"mapples_show_permission_additional_info_modal\"](this)></i>',
            "owner": files[j]['owner'],
            "row-style": row_style,
            "cell-style": cell_style
          });
      ...
      var output = support_scripts['mapples_create_table']([{"name":"", "size":"1em"}, {"name":"name", "size":"30em"},{"name":"size", "size":"6em"},{"name":"last accessed", "size":"8em"},{"name":"last modified", "size":"8em"},{"name":"creation date", "size":"8em"},{"name":"acls", "size":"1em"}, {"name":"owner", "size":"15em"}], rows);
      output += support_scripts['mapples_create_permission_additional_info_modal'](uniqueName);
      return output;
    } else {
      ...
      for (var j = 0; j < files.length; j++)
      {
        output += files[j]['creation_date'] + "," + files[j]['modify_time'] + "," + files[j]['access_time'] + "," + files[j]['owner'] + "," + support_scripts['mapples_file_size_to_human_readable_string'](files[j]['size']) + "," + files[j]['name'] + "\n";
      }
      output += "</pre>"
      return output;
    }
